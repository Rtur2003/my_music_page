...
const SKILL_ICONS = { programming: Code, web: Globe, ai: Cpu, tools: Wrench };
        ...
        <div className={styles.grid}>
          ...
          <div className={`${styles.skillsGrid} ${styles.reveal}`}>
            {skillKeys.map((key) => {
              ...
              const skill = t(`about.skills.${key}`);
              return (
                <div key={key} className={styles.skillCard}>
                  <Icon size={22} strokeWidth={1.5} className={styles.skillIcon} />
                  <div className={`${styles.skillTitle} font-display`}>{skill.title}</div>
                  <div className={styles.skillDetail}>{skill.detail}</div>
                </div>
              );
            })}
          </div>
